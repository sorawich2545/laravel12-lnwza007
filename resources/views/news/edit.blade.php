...
@section('description', 'แก้ไขข่าวภาพยนตร์')

@section('content')
  <!-- Hero Section -->
  <section id="hero" class="hero section dark-background">
    <img src="{{ asset('assets/img/img_h_1.jpg') }}" alt="Edit News Hero" data-aos="fade-in">
    <div class="container">
      <div class="row justify-content-center text-center" data-aos="fade-up" data-aos-delay="100">
        <div class="col-xl-6 col-lg-8">
          <h2>แก้ไขข่าว<span>.</span></h2>
          <p>แก้ไขข้อมูลข่าวภาพยนตร์</p>
        </div>
      </div>
    </div>
  </section>
              ...
              <form action="{{ route('news.update', $article->id) }}" method="POST">
                ...
                <div class="row">
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('movie_title') is-invalid @enderror" 
                           id="movie_title" name="movie_title" value="{{ old('movie_title', $article->movie_title) }}" required>
                    @error('movie_title')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  </div>
                ...
                <div class="row">
                  <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('genre') is-invalid @enderror" id="genre" name="genre" required>
                      ...
                      <option value="Action" {{ old('genre', $article->genre) == 'Action' ? 'selected' : '' }}>Action</option>
                      <option value="Comedy" {{ old('genre', $article->genre) == 'Comedy' ? 'selected' : '' }}>Comedy</option>
                      <option value="Drama" {{ old('genre', $article->genre) == 'Drama' ? 'selected' : '' }}>Drama</option>
                      <option value="Horror" {{ old('genre', $article->genre) == 'Horror' ? 'selected' : '' }}>Horror</option>
                      <option value="Romance" {{ old('genre', $article->genre) == 'Romance' ? 'selected' : '' }}>Romance</option>
                      <option value="Sci-Fi" {{ old('genre', $article->genre) == 'Sci-Fi' ? 'selected' : '' }}>Sci-Fi</option>
                      <option value="Thriller" {{ old('genre', $article->genre) == 'Thriller' ? 'selected' : '' }}>Thriller</option>
                      <option value="Animation" {{ old('genre', $article->genre) == 'Animation' ? 'selected' : '' }}>Animation</option>
                      <option value="Documentary" {{ old('genre', $article->genre) == 'Documentary' ? 'selected' : '' }}>Documentary</option>
                    </select>
                    @error('genre')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  ...
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="date" class="form-control @error('release_date') is-invalid @enderror" 
                           id="release_date" name="release_date" value="{{ old('release_date', $article->release_date->format('Y-m-d')) }}" required>
                    @error('release_date')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  </div>
                ...
                <div class="row">
                  <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('director') is-invalid @enderror" 
                           id="director" name="director" value="{{ old('director', $article->director) }}" required>
                    @error('director')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  </div>
                ...
                <div class="mb-3">
                  ...
                  <input type="url" class="form-control @error('reference_link') is-invalid @enderror" 
                         id="reference_link" name="reference_link" value="{{ old('reference_link', $article->reference_link) }}" 
                ...
                <div class="mb-3">
                  ...
                  <div class="form-text">อักขระที่ใช้: <span id="summary-count">{{ strlen(old('summary', $article->summary)) }}</span>/500</div>
                  @error('summary')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle"></i> อัปเดตข่าว
                  </button>
                </div>
              </form>
            </div>
